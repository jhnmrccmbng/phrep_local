                        ...
                        <?php }
                            ...
                            <div class="panel-body">
                              <form action = "sec_dashboard_action.php" method = "POST" enctype = "multipart/form-data" class="form">
                                  ...
                                  <h4>Attached Disapproval Letter:</h4>       
                                            <?php 
                                                $where = array("username" => $mi['username']);
                                                $getUserID = $obj->getUser("phrepuser", $where);
                                                if($getUserID){
                                            ...
                                            ?>
                                            <input id="times" name="times" type="hidden" value="1" placeholder="" class="form-control input-md">
                                            <input id="docname" name="docname" type="hidden" value="DisapprovalLetter" placeholder="" class="form-control input-md">
                                            <input id="date" name="date" type="hidden" value="<?php $date;?>" placeholder="" class="form-control input-md">
                                            <input id="doctype" name="doctype" type="hidden" value="37" placeholder="" class="form-control input-md">
                                            <input id="submid" name="submid" type="hidden" value="<?php echo $_GET['id'];?>" placeholder="" class="form-control input-md">
                                            <input id="kind" name="kind" type="hidden" value="DAL" placeholder="" class="form-control input-md">
                                            <?php $actual_link = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; ?>
                                            <input id="urllink" name="urllink" type="hidden" value="<?php echo $actual_link;?>" placeholder="" class="form-control input-md">
                                            <input id="username" name="username" type="hidden" value="<?php echo $mi['username'];?>" placeholder="" class="form-control input-md">
                                            
                                            <?php
                                            $getmaxec = $obj->getMaxRevisions($id, "DAL");
                                            echo '<input name="revision" type="hidden" value="'.$getmaxec.'">';
                                            ?>
                                            ...
                                            <div class="row">
                                                <div class="col-lg-12">
                                                    <span class="btn btn-default btn-file">
                                                    <input id="attach" type = "file" name = "disapproval" />  
                                                    </span>                      
                                                    <input id="attached" type = "submit" name="disapproval" class="btn btn-primary"/>
                                                </div>
                                            </div>
                                        </form>
                                
                                <?php 
                                $where = array("sub_id" => $id, "doctype" => "37", "kind" => "DAL", "finaldoc" => "1");
                                $getclerance = $obj->fetch_record_with_where("document", $where);
                                if($getclerance){
                                    foreach($getclerance as $clear){
                                       echo '<br><a class="btn btn-default btn-xs" href="sec_dashboard_action.php?deleteccccc=1&id='.$clear['file_id'].'&subid='.$clear['sub_id'].'" role="button"><span data-toggle="tooltip" title="DELETE" class="glyphicon glyphicon-remove" aria-hidden="true"></span></a> '.$clear['orig_filename'];
                                       echo ' <input id="ecpath" name="ecpath" type="hidden" value="'.$clear['path'].'">';
                                       $ecpath = $clear['path'];
                                    }
                                }
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-8"><center><div class="alert alert-danger" role="alert">Please double check before disapproving.</div>
                                                <input name="submid" type="hidden" value="<?php echo $_GET['id'];?>">    
                                                <input name="userid" type="hidden" value="<?php echo $userid;?>"> 
                                                <?php 
                                                $getMaxRevisionEC = $obj->getMaxRevisions($id, "DAL");
                                                $whereec = array("sub_id" => $id, "kind" => "DAL", "finaldoc"=> '1', "revision" => $getMaxRevisionEC);
                                                $ec = $obj->fetch_record_with_where("document", $whereec);
                                                if($ec){$ecc = "";} else{$ecc = "disabled";}
                                                ?>    
                                                    <button name="disapprovedreview" type="submit" class="btn btn-danger" <?php echo $ecc;?>>DISAPPROVED</button> </center>
                                            </div>
                                            <div class="col-lg-2">
                                            </div>
                                        </div>
